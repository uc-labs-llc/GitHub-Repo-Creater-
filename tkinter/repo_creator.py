...
class GitHubRepoCreator:
    ...
    def create_repo(self):
        ...
        try:
            ...
            self.status.config(text=f"Error: {error_msg}", fg="red")
...
# git push {form_data['gitRemoteName'] or 'origin'} {form_data['gitBranchName'] or 'main'}
        ...
        # Save in user's Downloads directory
        downloads_dir = os.path.join(os.path.expanduser("~"), "Downloads")
        if not os.path.exists(downloads_dir):
            os.makedirs(downloads_dir)  # Create Downloads folder if it doesn’t exist
